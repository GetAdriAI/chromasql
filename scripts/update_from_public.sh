...
#
# This script pulls the latest contents of the public chromasql repository and
# synchronizes them into the local chromasql/ directory so the private repo
# stays aligned.
#
# Steps performed:
#   1. Determine the remote/branch to fetch (defaults: chromasql-public/main).
#   2. Fetch the remote branch into the local Git repository.
#   3. Export the branch into a temporary directory.
#   4. Rsync the exported files into chromasql/ (preserving documentation files).
...
Usage: update_from_public.sh [-r remote] [-b branch] [--archive]
  ...
  -r, --remote   Remote name of the public repo (default: chromasql-public)
...
REMOTE="${UPDATE_REMOTE:-chromasql-public}"
...
TMP_DIR="$(mktemp -d -t chromasql-public-XXXXXX)"
...
echo "Synchronizing into chromasql/ ..."
rsync -a --delete \
  --exclude '.git' \
  --exclude '.github/' \
  --exclude '__pycache__/' \
...
git -C "${REPO_DIR}" status -- chromasql

echo "Remember to run 'git add chromasql && git commit' once you're satisfied."
