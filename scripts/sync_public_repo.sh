...
Usage: sync_public_repo.sh [-r remote] [-b branch] [--tag]
  ...
  -r, --remote   Git remote to push to (default: chromasql-public)
...
REMOTE="${SYNC_REMOTE:-chromasql-public}"
...
TMP_DIR="$(mktemp -d -t chromasql-sync-XXXXXX)"
...
if git -C "${TMP_DIR}" diff --quiet --cached; then
  ...
  git -C "${TMP_DIR}" commit -m "Sync chromasql from ${SRC_SHA}"
...
PY
  ...
  if git -C "${TMP_DIR}" ls-remote --exit-code --tags origin "${TAG}" >/dev/null 2>&1; then
    ...
    git -C "${TMP_DIR}" tag -a "${TAG}" -m "chromasql ${VERSION}"
